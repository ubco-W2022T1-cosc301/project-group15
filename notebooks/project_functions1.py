...
def dataloadnclean(path):
    ...
    dataset = (
        ...
        .assign(all_reviews = lambda x: x['all_reviews'].apply(lambda y: re.findall("(\d{1,3}%)", y)[0]))
        [dataset['recent_reviews'].notna() & dataset['recent_reviews'].str.contains('%')]
        .assign(recent_reviews = lambda x: x['recent_reviews'].apply(lambda y: re.findall("(\d{1,3}%)", y)[0]))
    )
    return dataset